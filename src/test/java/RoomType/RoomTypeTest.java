...
public class RoomTypeTest {
	...
	public void RoomTypePostTest() {
		...
		RoomType mockrt = mock(RoomType.class);
		when(mockrt.getId()).thenReturn(1);

		//RoomType rt = new RoomType("Lecture Hall");
		//when(rtDAO.makePersistent(rt)).thenReturn(rt);
		Integer rPost = RoomTypeController.postRoomType(mockrt, req, res);
	...
	public void RoomTypeDeleteTest() {
		...
		when(rtDAO.findById(1)).thenReturn(mockroomType);
		RoomTypeController.deleteRoomType(1, req, res);
		verify(rtDAO, times(1)).makeTransient(mockroomType);
	}
